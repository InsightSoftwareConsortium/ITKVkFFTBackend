...
itkVkForwardInverse1DFFTImageFilterTest(int argc, char * argv[])
{
  ...
  bool testsPassed{ true };
  {
    ...
    for (int mySize = 2; mySize <= 20; ++mySize)
    {
      ...
      using ForwardFilterType = itk::VkForward1DFFTImageFilter<RealImageType>;
      typename ForwardFilterType::Pointer forwardFilter{ ForwardFilterType::New() };
      if (firstPass)
      {
        ITK_EXERCISE_BASIC_OBJECT_METHODS(forwardFilter, VkForward1DFFTImageFilter, Forward1DFFTImageFilter);
      }
      ...
      using InverseFilterType = itk::VkInverse1DFFTImageFilter<ComplexImageType>;
      typename InverseFilterType::Pointer inverseFilter{ InverseFilterType::New() };
      if (firstPass)
      {
        ITK_EXERCISE_BASIC_OBJECT_METHODS(inverseFilter, VkInverse1DFFTImageFilter, Inverse1DFFTImageFilter);
      }
